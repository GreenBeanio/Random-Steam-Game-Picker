...
def GetID(vanity_url, api_key):
    API_Request = Steam_Web_API_ID_URL \
        +"?"+"key="+api_key \
...
def CreateInfo_Start(config_info): #1
    ...
    with open(directory_path_config, "w") as outputfile:
        json.dump(info, outputfile, sort_keys=False, indent=4) #Format and write JSON to file
    return info
...
def Get_Games(games):
    ...
    API_Request = Steam_Web_API_Game_URL \
    +"?"+"key="+key \
    +"&"+"steamid="+steamid \
    ...
    for x in raw_games: #Going through all games in the JSON
        ...
        game_time_hours = round(game_time/60,2) #Converting the game time into hours
        played_games[game_title] = game_time_hours #Adding the games and game times to the dictionary
    ### Sorting the Dictionary by Hours ###
    games_ordering = sorted(played_games.items(), key=lambda x:x[1],reverse=True) #Sorting the games by their hours
    ...
    with open(directory_path_purge,'r') as f:
        purge_list = f.read().splitlines() #Reading the text file with games to not include
    ...
    with open(directory_path_games, "w") as outputfile:
        json.dump(purged_games, outputfile, sort_keys=False, indent=4) #Format and write JSON to file
...
def GetProfile(username):
    API_Request = Steam_Web_API_Player_URL \
        +"?"+"key="+key \
        +"&"+"steamids="+steamid
    ### Calling the API ###
    response_API = requests.get(API_Request)
...
def First_Choice(games):
    ...
    User_Choice = input("Do you want to call the API or Load from a previous file? (1 or 2):\n")
...
def Remove_Played_Games(games):
    ...
    for x in Game_List: #Checking every game in the game list
        ...
        if(play_time == 0):
            unplayed_games[x] = play_time #Adding games with no playtime to the dictionary
...
def Second_Choice(games):
    system("cls")
    get_games = {} #Dictionary to hold the games from the users choice
    User_Choice = input("Do you want to seperate out games with no playtime? (1 or 2):\n")
    User_Decision = int(User_Choice)
    if(User_Decision == 1):
...
def Get_Game_Stats(games):
    ...
    output = "Username: " + str(steam_name) + "\nId: " + str(steamid) + "\n------------------------" + \
    "\nTotal Hours: " + str(Total_Hours) + "\n------------------------" + \
    "\nTotal Games: " + str(Total_Games) + "\n------------------------" + \
    "\nPlayed Games: " + str(Played_Games) + "\nUnplayed Games: " + str(Unplayed_Games) + "\n------------------------" + \
    "\nPlayed Games Breakdown: " + "\nOver 1000: " + str(More_Than_1000) + "\nBetween 500 & 1000: " + str(Between_500_1000) + \
    "\nBetween 250 & 500: " + str(Between_250_500) + "\nBetween 100 & 250: " + str(Between_100_250) + \
    "\nBetween 50 & 100: " + str(Between_50_100) + "\nBetween 25 & 50: " + str(Between_25_50) + \
    "\nBetween 20 & 25: " + str(Between_20_25) + "\nBetween 15 & 20: " + str(Between_15_20) + \
    "\nBetween 10 & 15: " + str(Between_10_15) + "\nBetween 5 & 10: " + str(Between_5_10) + \
    "\nBetween 1 & 5: " + str(Between_1_5) + "\nLess Than 1: " + str(Less_Than_1) 
...
system("cls")
print(Player_Stats)
input('Press Enter to Exit') # Prevent the application from closing until an input is given